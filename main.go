...
func init() {
	...
	rootCmd.Flags().BoolVar(&presenter, "presenter", false, "run the speaker view with notes, next-slide preview, and elapsed timer; opens a socket for viewers to connect (implies -P)")
	rootCmd.Flags().BoolVar(&viewer, "viewer", false, "connect to a --presenter and render clean slides for the audience (implies -P)")
